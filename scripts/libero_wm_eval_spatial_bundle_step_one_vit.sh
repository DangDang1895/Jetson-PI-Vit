...
PER_STEP_CEREBELLUM="${LIBERO_WM_EVAL_PER_STEP_CEREBELLUM:-0}"
PER_STEP_HANDOVER_MODE="${LIBERO_WM_EVAL_PER_STEP_HANDOVER_MODE:-tail_append}"

WM_VISUAL_CONDITION_KIND="${WM_VISUAL_CONDITION_KIND:-residual}"
WM_ACTION_ENCODER_KIND="${WM_ACTION_ENCODER_KIND:-transformer_block}"
WM_NUM_REDUCER_HEADS="${WM_NUM_REDUCER_HEADS:-4}"
WM_NUM_FUTURE_HEADS="${WM_NUM_FUTURE_HEADS:-4}"
WM_GRU_HIDDEN_DIM="${WM_GRU_HIDDEN_DIM:-256}"
WM_GRU_NUM_LAYERS="${WM_GRU_NUM_LAYERS:-2}"
WM_TRANSFORMER_NUM_HEADS="${WM_TRANSFORMER_NUM_HEADS:-4}"
WM_TRANSFORMER_FFN_MULTIPLIER="${WM_TRANSFORMER_FFN_MULTIPLIER:-4}"

if [[ "${PER_STEP_CEREBELLUM}" == "1" && "${AH}" != "10" ]]; then
  echo "Per-step cerebellum currently requires AH=10, got AH=${AH}" >&2
  exit 1
fi
...

{
  ...
  echo "PER_STEP_CEREBELLUM=${PER_STEP_CEREBELLUM}"
  echo "PER_STEP_HANDOVER_MODE=${PER_STEP_HANDOVER_MODE}"
  echo "PORT=${PORT}"
...
SERVE_WM_ARGS=(
  ...
  --world-model-action-encoder-kind "${WM_ACTION_ENCODER_KIND}"
  --world-model-visual-condition-kind "${WM_VISUAL_CONDITION_KIND}"
  --world-model-num-reducer-heads "${WM_NUM_REDUCER_HEADS}"
  --world-model-num-future-heads "${WM_NUM_FUTURE_HEADS}"
  --world-model-gru-hidden-dim "${WM_GRU_HIDDEN_DIM}"
  --world-model-gru-num-layers "${WM_GRU_NUM_LAYERS}"
  --world-model-transformer-num-heads "${WM_TRANSFORMER_NUM_HEADS}"
  --world-model-transformer-ffn-multiplier "${WM_TRANSFORMER_FFN_MULTIPLIER}"
)
if [[ "${NO_WM}" == "1" ]]; then
  SERVE_WM_ARGS=()
fi
...
if [[ "${PER_STEP_CEREBELLUM}" == "1" ]]; then
  if [[ ${#EXTRA_MAIN_FLAGS[@]} -ne 0 ]]; then
    echo "Per-step cerebellum cannot use old adaptive/multi-rollout flags." >&2
    exit 1
  fi

  MAIN_ASYNC_ARGS=(
    --async-per-step-cerebellum
    --per-step-handover-mode
    "${PER_STEP_HANDOVER_MODE}"
  )
else
  MAIN_ASYNC_ARGS=(
    --async-inference
